...
All mutator classes roughly look like this:

.. code-block:: python3

    class Dummy:
        """Does some clever simplification."""
        def filter(self, node):
            """Check whether this mutators can be applied to the given node.
            If not specified, we assume ``True``."""
            return True
...
Please carefully read the section on :ref:`writing new mutators <Writing new mutators>` if you want to implement new mutators.
